...
class FleetLogixETL:
    ...
    def load_dimensions(self, df: pd.DataFrame):
        """Cargar clientes en Snowflake mediante inserción por lote."""
        ...
        try:
            customers = (
                df[["customer_name", "destination_city"]]
            ...
            self.sf_conn.commit()
            logging.info(
                f" Dimensiones actualizadas: {len(customer_data)} clientes cargados"
            )
        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS daily_delivery_totals (
                    ...
                    total_weight_kg NUMBER(18,2),
                    total_revenue NUMBER(18,2),
                    average_delivery_time_minutes NUMBER(10,2),
                    etl_batch_id INT,
            ...
            cursor.execute("""
                ...
                SELECT
                    ...
                    COUNT_IF(is_on_time = FALSE),
                    COALESCE(SUM(package_weight_kg), 0),
                    COALESCE(SUM(revenue_per_delivery), 0),
                    COALESCE(AVG(delivery_time_minutes), 0),
                    %s
